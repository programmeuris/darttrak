...
import type { GameType, Match, Leg, Player } from '../types';
...
const SUPPORTED_GAMES: { type: GameType; label: string; enabled: boolean }[] = [
  ...
  { type: 'AroundTheClock', label: 'Around the Clock', enabled: false },
];
...
export async function renderSetup(root: HTMLElement): Promise<void> {
  ...
  async function startMatch(): Promise<void> {
    ...
    // Reference startingScore to keep x01 assumptions explicit.
    void startingScore(state.gameType === '301' ? '301' : '501');
  ...
  mount(
    ...
    el('div', { class: 'screen' }, [
      ...
      el('button', { class: 'btn primary big full', onClick: startMatch }, [
        'Start Match',
      ]),
    ]),
  );
